...
object UserDefinedFunctionScala {
  ...
  def main(args: Array[String]): Unit = {
    ...
    eventsDS.createOrReplaceTempView("eventsDS")

    session
      .sql("SELECT get_json_object(e.device,'$.device_id') as deviceId, " +
        " get_json_object(e.device,'$.device_type') as deviceType, " +
        " get_json_object(e.device,'$.ip') as ip, " +
        " get_json_object(e.device,'$.cca3') as cca3 " +
        " FROM eventsDS e ")
      .show(false)

    //scalar function working on a single row
    val plusFive = functions.udf((x: Int) => x + 5);
    session.udf.register("plusFive", plusFive);

    session
      .sql("SELECT plusFive(e.id), get_json_object(e.device,'$.device_id') as deviceId " +
        " FROM eventsDS e").show(false)
  }
